...
export function GenerativeArtScene() {
    ...
    useEffect(() => {
        ...
        // Resolve color from CSS variable if possible, otherwise fallback
        const getCssVar = (name: string) => {
            const val = getComputedStyle(document.documentElement).getPropertyValue(name).trim();
            // Handle comma separated values for HSL if needed, but THREE.Color handles "hsl(h, s%, l%)"
            // If the var is "203, 92%, 53%", we need to wrap it in hsl()
            if (val && val.includes(',')) return `hsl(${val})`;
            if (val) return val; // assume it's a color string
            return "hsl(203, 92%, 53%)"; // fallback
        };
        ...
        const material = new THREE.ShaderMaterial({
            uniforms: {
                ...
                color: { value: new THREE.Color(getCssVar('--sky-300')) },
            },
